...
import type { Form } from '@formiz/core';
...
export const useRepeater = ({
  ...
  const [keys, setKeys] = useState<string[]>(initialValues.map(() => uuidv4()));
  ...
  const insertMultiple = useCallback((index: number, data?: Data[]): void => {
    setKeys((oldKeys) => {
      ...
      const getFromCollectionToUpdateRef = (_: any, localIndex: any) =>
        collectionToUpdateRef.current?.[localIndex];
      const newValues = [
        ...(oldKeys || [])
          .slice(0, computedIndex)
          .map(getFromCollectionToUpdateRef),
        ...dataToInsert,
        ...(oldKeys || [])
          .slice(computedIndex)
          .map(getFromCollectionToUpdateRef),
  ...
  const set = useCallback((values: any) => {
    setKeys(() => {
      const newKeys = values.map(() => uuidv4());
  ...
  useEffect(() => {
    if (collectionToUpdateRef.current) {
      formRef.current.setFieldsValues(
        lodashSet({}, name, collectionToUpdateRef.current)
      );
    }
